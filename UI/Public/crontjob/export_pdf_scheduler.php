...
/*
  Example : takes as a parameter a sql query and redirect the output to a file.
  
 ./export_pdf_scheduler.php id_scheduler > name_pdf_file
...
if (strlen($argv[1]) > 0){
	...
	$QUERY="SELECT subject,report FROM ".$FG_TABLE_NAME." WHERE id=".$argv[1];
	...
	$pdf->query($QUERY);
	$row = $pdf->get_row();
	$attr=array('titleFontSize'=>18,'titleText'=>'Asterisk CDR '.$row[0]);
	$pdf->sql_report($row[1],false,$attr);
}
?>
